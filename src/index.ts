...
const pageHanlder = (action: any, page: any) => {
	if (action === 'pageClick') {
		currentPage = page;
	}
	if (action === 'nextPage') {
...
const onChooseGenre = (genreId: any) => {
...
const renderMovie = (movies: any) => {
	...
	movies.forEach((movie: any) => {
		movieWrapper.innerHTML += `<div onclick="onMovieClick(${movie.movieId})" class="movie-card">
...
const renderGenres = () => {
	genres.forEach((genre: any) => {
		...
		<input type="checkbox" onclick="onChooseGenre(${genre.id})" />
...
const renderPages = (totalPages: any, action: any) => {
	...
	if (
		!paginationNumberArr.includes(currentPage) &&
		action === 'nextPage' &&
	...
	if (
		!paginationNumberArr.includes(currentPage) &&
		(action === 'previousPage' || action === 'pageClick')
	) {
		if (currentPage === 4) {
			pageIndex = 0;
		} else {
	...
	for (let i = 0; i < paginationNumberArr.length; i++) {
		const markup = `<button onclick="pageHanlder('pageClick', ${
